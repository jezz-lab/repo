...
-- ========== CONFIGURATION ==========
local TARGET_GAME_ID = 96645548064314  -- Set to a number (e.g., 1234567890) to restrict to one game, or nil to run anywhere.
...
local function createGUI()
    -- Toggle Icon (always visible, draggable)
    ...
    toggleFrame.Position = UDim2.new(0, 10, 0, 10)  -- top-left corner
    ...
    screenGui.Enabled = false  -- hidden until toggle clicked

    local mainFrame = Instance.new("Frame")
    mainFrame.Name = "MainFrame"
    mainFrame.Size = UDim2.new(0, 300, 0, 445)
    mainFrame.Position = UDim2.new(0, 60, 0, 10)  -- next to toggle icon
...
local function autoLoop()
    ...
    while autoEnabled do
        ...
        if potion and tier and claimPotionEvent then
            ...
            if getPlayerCash() < requiredCash then
                setStatus(string.format("❌ Need %s cash (have %s)", formatCash(requiredCash), formatCash(getPlayerCash())), Color3.fromRGB(255,100,100))
            ...
            while isBrewing and autoEnabled do
                ...
                if remaining <= 0 then
                    ...
                    setStatus(string.format("⏳ Brewing %s (%s left) 💰%s | 🧪 %d/3",
                        potionName, timeLeft, formatCash(requiredCash), potionCount), Color3.fromRGB(255,200,50))
                    wait(1)
                end
            end
...
-- ========== WIRE UP UI EVENTS ==========
-- Toggle icon: show/hide main GUI
