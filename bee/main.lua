...
local startupSuccess, startupError = xpcall(function()
	...
	-- Handler for bee.fillingStarted event (Primary for all 3)
	...
	if BeeFillingStarted then
		BeeFillingStarted.OnClientEvent:Connect(
			function(dispenserId, data, arg3)
				local success, err = xpcall(
					function()
						...
						if type(data) ~= "table" then
							error("Bee event data is not a table")
						end

						if type(data.fishes) ~= "table" then
							error("Bee event is missing the fishes table")
						end

						local fish1 = data.fishes[1]
						local fish2 = data.fishes[2]

						if not fish1 or not fish2 then
							error("Bee event does not contain 2 fish requirements")
						end
						...
						local available = safeUpdateRequirement(dispenserId)

						if available == nil then
							error("Requirement display could not be updated")
						end
					end,
				...
				if not success then
					notifyFailure("Bee Requirement Update", err)
				end
			end
		)
	else
		warn("[ActionGui] bee.fillingStarted RemoteEvent not found")
	end

	-- Handler for state.sync event (Fallback for all 3 if bee.fillingStarted fails)
		...
		StateSync.OnClientEvent:Connect(
			function(data)
				local success, err = xpcall(
					function()
						if type(data) ~= "table" then
							error("State sync data is not a table")
						end
						...
						local playersData = data.data["playerData/players"]
						...
						local beeData = playerData.events.Bee
						if not beeData then
							return
						end

						if not beeData.recipes then
							return
						end

						-- Update each dispenser from the recipes (All 3)
						for dispenserId, recipe in pairs(beeData.recipes) do
							if dispenserNames[dispenserId] then
								if type(recipe.fishes) == "table" then
									local fish1 = recipe.fishes[1]
									local fish2 = recipe.fishes[2]

									if fish1 and fish2 then
										fishRequirements[dispenserId] = {
											tostring(fish1),
											tostring(fish2)
										}
						...
						-- Update the display for all dispensers
						for dispenserId = 1, 3 do
							safeUpdateRequirement(dispenserId)
						end
					end,
					function(errorMessage)
						return debug.traceback(tostring(errorMessage), 2)
				...
				if not success then
					notifyFailure("State Sync Update", err)
				end
			end
		)
	else
		warn("[ActionGui] state.sync RemoteEvent not found")
	end
	...
	-- Check current requirements on startup
	for dispenserId = 1, 3 do
		local success, err = xpcall(
			function()
				local available =
	...
	--==================================================
	-- SIMULATE EVENTS FOR ALL THREE DISPENSERS
	--==================================================

	-- Primary: bee.fillingStarted for ALL THREE dispensers
	local function simulateBeeFillingEvents()
		local BeeFillingStarted = ReplicatedStorage
			.rbxts_include
			.node_modules["@rbxts"]
			.remo
		...
		if BeeFillingStarted then
			-- Dispenser 1 (Left)
			pcall(function()
				BeeFillingStarted:FireServer(
					1,
					{
						expiresAt = 1788092607,
						baitRarity = "Epic",
						reward = 29,
						fishes = {
							"BlueSpottedPuffer",
	...
	-- Fallback: state.sync for ALL THREE dispensers
